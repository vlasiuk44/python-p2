...
async def test_register_login_me_flow(test_client: AsyncClient):
    register_resp = await test_client.post(
        "/auth/register",
        json={"email": "vlasiuk@example.com", "password": "pass1234"},
    )
    assert register_resp.status_code == 201

    login_resp = await test_client.post(
        "/auth/login",
        data={"username": "vlasiuk@example.com", "password": "pass1234"},
    )
...
async def test_login_with_wrong_password_returns_401(test_client: AsyncClient):
    ...
    login_resp = await test_client.post(
        "/auth/login",
        data={"username": "vlasiuk@example.com", "password": "wrong-password"},
    )
